...
{
    public partial class SkydriveBrowseSave : PhoneApplicationPage
    {
        ...
        {
            var x = contentList.SelectedItem as SkyDriveContent;
            if (x != null)
            {
                if (x.Type == "folder")
                {
                    this.NavigationService.Navigate(new Uri(string.Format("/SkydriveBrowseSave.xaml?folderId={0}&displayName={1}&fileName={2}&fullPath={3}\\{1}&listName={4}&xmlFile={5}", x.Id, x.Name, this.txtFileName.Text, this.txtFullPath.Text, this.PageTitle.Text, _xmlFile), UriKind.Relative));
                }
                else
                {
